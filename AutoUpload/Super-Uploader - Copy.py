...
def do_gemini_format(raw_text, gemini_model, base_logs):
            ...
            try:
                ...
                if temp_text:
                    ...
                    if len_output > 1.65 * len_input:
                        ...
                        failed_path = os.path.join(SCRIPT_DIR, f"failed_{model_name}.txt")
                        try:
...
def execute_pipeline(op_mode, scope, single_audio, multi_audio, folder_input, 
        ...
        for part_dir in part_dirs:
            ...
            if part_raw_transcripts:
                ...
                for c_idx, chunk in enumerate(txt_chunks, start=1):
                    ...
                    yield logs
                    formatted_chunk, logs = do_gemini_format(chunk, gemini_model, logs)
                    yield logs
    ...
    # ---------------------------------------------------------
        ...
        def process_gemini(raw_text, output_base_name, current_logs):
            if len(raw_text) > 15000:
                ...
                for idx, chunk in enumerate(txt_chunks, start=1):
                    # Output chunk as text
                    chunk_raw_path = f"{output_base_name}_Raw_Chunk_{idx:03d}.txt"
                    ...
                    formatted_chunk, current_logs = do_gemini_format(chunk, gemini_model, current_logs)
                    
                    if formatted_chunk:
                        # Output formatted chunk as text
                        chunk_formatted_path = f"{output_base_name}_Formatted_Chunk_{idx:03d}.txt"
                ...
                formatted_txt, current_logs = do_gemini_format(raw_text, gemini_model, current_logs)
                
            if formatted_txt:
                txt_out = f"{output_base_name}_Formatted.txt"
                current_logs = write_file_with_log(txt_out, formatted_txt, current_logs)
                if HAS_FORMATTER:
                    doc_path = f"{output_base_name}_Final.docx"
                    try:
                        format_document(txt_out, WORD_TEMPLATE_PATH, doc_path)
        ...
        if scope == "Singular":
            ...
            if single_txt:
                ...
                input_base_name = os.path.splitext(os.path.basename(single_txt.name))[0]
                output_base = os.path.join(SCRIPT_DIR, input_base_name)
            else:
                output_base = os.path.join(SCRIPT_DIR, f"Gemini_{int(time.time())}")
            if combined_text.strip():
                logs = update_log(logs, "⚙️ Processing Gemini singular text...")
                yield logs
                logs = process_gemini(combined_text, output_base, logs)
                yield logs
            ...
            for f in multi_txt:
                try:
                    ...
                    input_base_name = os.path.splitext(os.path.basename(f.name))[0]
                    base = os.path.join(SCRIPT_DIR, input_base_name)
                    logs = update_log(logs, f"⚙️ Processing Gemini bulk file: {f.name}...")
                    yield logs
                    logs = process_gemini(text_data, base, logs)
                    yield logs
                except Exception as e:
