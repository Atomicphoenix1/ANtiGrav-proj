...
DASHSCOPE_MODELS = {
    ...
    "GLM (Zhipu)": ["glm-5.1", "glm-5"],
    "Kimi (Moonshot)": ["kimi-k2.6", "kimi-k2.5"],
    "DeepSeek": ["deepseek-v3.1", "deepseek-r1"],
    "MiniMax": ["MiniMax-M2.7", "MiniMax-M2.5"],
    # ADD MORE DASHSCOPE MODELS HERE
    # "Provider Name": ["model-id-1", "model-id-2"],
}

ALL_DASHSCOPE_MODELS = [m for group in DASHSCOPE_MODELS.values() for m in group]
ALL_AVAILABLE_MODELS = GEMINI_MODELS + ALL_DASHSCOPE_MODELS
...
def do_gemini_format(raw_text, gemini_model, base_logs, output_folder=None, chunk_idx=1, result_holder=None):
    ...
    fallback_matrix = [gemini_model] + [m for m in GEMINI_MODELS if m != gemini_model]
    formatted_text = None
...
def do_dashscope_format(raw_text, dashscope_model, base_logs, output_folder=None, chunk_idx=1, result_holder=None):
    fallback_matrix = [dashscope_model] + [m for m in ALL_DASHSCOPE_MODELS if m != dashscope_model]
    formatted_text = None
    prompt = get_gemini_prompt(raw_text)
    len_input = len(raw_text)

    for model_name in fallback_matrix:
        base_logs = update_log(base_logs, "🔄 Engaging DashScope Model: " + model_name + "...")
        yield base_logs
        for attempt in range(1, 3):
            try:
                base_logs = update_log(base_logs, "   ⏳ [Attempt " + str(attempt) + "/2] Strict Time-Bomb Active (3.5 Mins)...")
                yield base_logs
                chat = dashscope_client.chat.completions.create(
                    model=model_name,
                    messages=[{"role": "user", "content": prompt}],
                    temperature=0.1
                )
                temp_text = chat.choices[0].message.content
                if temp_text:
                    len_output = len(temp_text)
                    if len_output > 1.65 * len_input:
                        base_logs = update_log(base_logs, "   ✅ SUCCESS! Character count check PASSED: output (" + str(len_output) + ") > 1.65x input (" + str(len_input) + ").")
                        formatted_text = temp_text
                        yield base_logs
                        break
                    else:
                        base_logs = update_log(base_logs, "   ⚠️ SUCCESS but FAILED character count check: output (" + str(len_output) + ") is NOT strictly above 1.65x input (" + str(len_input) + "). harakat/tags might be incomplete. Saving failed output for inspection...")
                        yield base_logs
                        target_dir = output_folder if output_folder else SCRIPT_DIR
                        failed_path = os.path.join(target_dir, "failed_chunk" + str(chunk_idx) + ".txt")
                        try:
                            with open(failed_path, "w", encoding="utf-8") as f_fail:
                                f_fail.write(temp_text)
                            base_logs = update_log(base_logs, "   💾 Saved failed trial output to: " + os.path.basename(failed_path))
                        except Exception as save_err:
                            base_logs = update_log(base_logs, "   ❌ Failed to save trial output: " + str(save_err))
                        yield base_logs
                else:
                    base_logs = update_log(base_logs, "   ⚠️ Failed: Empty response received.")
                    yield base_logs

                if attempt < 2: time.sleep(3)
            except concurrent.futures.TimeoutError:
                base_logs = update_log(base_logs, "   ⚠️ Failed: CONNECTION TIMED OUT.")
                yield base_logs
                if attempt < 2: time.sleep(3)
            except Exception as e:
                base_logs = update_log(base_logs, "   ⚠️ Failed: " + str(e))
                yield base_logs
                if attempt < 2: time.sleep(3)
        ...
        if formatted_text:
            ...
            base_logs = update_log(base_logs, "❌ " + model_name + " failed to deliver valid fully-diacritized output. Switching models...")
            yield base_logs
...
def execute_pipeline(op_mode, scope, single_audio, multi_audio, folder_input, 
                     single_txt, multi_txt, text_box, device, provider, selected_model,
    ...
    # Select formatting function based on provider
    format_func = do_gemini_format if provider == "Gemini" else do_dashscope_format
        ...
        for part_dir in part_dirs:
            ...
            if part_raw_transcripts:
                ...
                for c_idx, chunk in enumerate(txt_chunks, start=1):
                    ...
                    res_holder = {}
                    for updated_logs in format_func(chunk, selected_model, logs, output_folder=part_dir, chunk_idx=c_idx, result_holder=res_holder):
                        logs = updated_logs
                        yield logs
    ...
    # ---------------------------------------------------------
        ...
        def process_gemini(raw_text, lesson_folder, lesson_name, part_dir, part_name, current_logs):
            ...
            for idx, chunk in enumerate(txt_chunks, start=1):
                ...
                res_holder = {}
                for updated_logs in format_func(chunk, selected_model, current_logs, output_folder=part_dir, chunk_idx=idx, result_holder=res_holder):
...
def toggle_inputs(op, scope, split):
    ...
    is_formatting_op = op in ["Gemini Only", "Whisper + Gemini"]
    ...
    return [
        ...
        gr.update(visible=is_formatting_op),  # provider_dd
        gr.update(visible=is_formatting_op),  # model_dd
    ]
...
with gr.Blocks(title="Super Uploader V8") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            provider_dd = gr.Radio(["Gemini", "Alibaba Cloud"], value="Gemini", label="AI Provider", visible=False)
            model_dd = gr.Dropdown(choices=GEMINI_MODELS, value="gemini-3-flash-preview", label="Model", visible=False)
    ...
    def update_models(provider):
        return gr.update(choices=GEMINI_MODELS, value=GEMINI_MODELS[0]) if provider == "Gemini" else gr.update(choices=ALL_DASHSCOPE_MODELS, value=ALL_DASHSCOPE_MODELS[0])
    ...
    submit_btn.click(
        ...
        inputs=[
            ...
            single_txt, multi_txt, text_box, device_dd, provider_dd, model_dd, 
