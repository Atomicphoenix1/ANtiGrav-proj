    ...
    for part in parts:
        ...
        if part.startswith('<hadith>') and part.endswith('</hadith>'):
            ...
            if content.startswith('﴿') and content.endswith('﴾'):
                ...
                run_qopen = run if not has_outer_brackets else para.add_run()
                run_qopen.text = '﴿'
                apply_formatting(run_qopen, base_font, 18, color=RGBColor(0x20, 0x21, 0x22))
                ...
                run_mid = run if not has_outer_brackets else para.add_run()
                run_mid.text = content
                apply_formatting(run_mid, 'KFGQPC Uthmanic Script HAFS', 18, color=RGBColor(0x20, 0x21, 0x22))
                
            if has_outer_brackets:
                run_close = para.add_run('<')
                apply_formatting(run_close, base_font, base_size, bold=False, color=base_color)
            ...
            sub_pattern = rf'(<hadith>.*?</hadith>|<quran>.*?</quran>|<strong>.*?</strong>|{honorific_subpattern})'
            if re.search(sub_pattern, content):
                sub_parts = re.split(sub_pattern, content)
                valid_sub_parts = [p for p in sub_parts if p]
                
                # Check if first/last need prefix/suffix
                for s_idx, sub_part in enumerate(valid_sub_parts):
                    is_sub_honorific = re.match(rf'^({honorific_subpattern})$', sub_part)
                    
                    prefix = ">" if s_idx == 0 else ""
                    suffix = "<" if s_idx == len(valid_sub_parts) - 1 else ""
                    
                    if sub_part.startswith('<quran>') and sub_part.endswith('</quran>'):
                        qcontent = sub_part.replace('<quran>', '').replace('</quran>', '')
                        if prefix:
                            run_pref = para.add_run(prefix)
                            apply_formatting(run_pref, base_font, base_size, bold=False, color=base_color)
                        
                        if qcontent.startswith('﴿') and qcontent.endswith('﴾'):
                            run_qopen = para.add_run('﴿')
                            apply_formatting(run_qopen, base_font, 18, color=RGBColor(0x20, 0x21, 0x22))
                            run_mid = para.add_run(qcontent[1:-1])
                            apply_formatting(run_mid, 'KFGQPC Uthmanic Script HAFS', 18, color=RGBColor(0x20, 0x21, 0x22))
                            run_qclose = para.add_run('﴾')
                            apply_formatting(run_qclose, base_font, 18, color=RGBColor(0x20, 0x21, 0x22))
                        else:
                            run_mid = para.add_run(qcontent)
                            apply_formatting(run_mid, 'KFGQPC Uthmanic Script HAFS', 18, color=RGBColor(0x20, 0x21, 0x22))
                            
                        if suffix:
                            run_suff = para.add_run(suffix)
                            apply_formatting(run_suff, base_font, base_size, bold=False, color=base_color)
                            
                    elif sub_part.startswith('<hadith>') and sub_part.endswith('</hadith>'):
                        hcontent = sub_part.replace('<hadith>', '').replace('</hadith>', '')
                        is_bold = True
                        if hcontent.startswith('[') and hcontent.endswith(']'):
                            hcontent = hcontent[1:-1]
                        
                        run_h = para.add_run(f"{prefix}{hcontent}{suffix}")
                        apply_formatting(run_h, base_font, base_size, bold=is_bold, color=base_color)
                        
                    elif sub_part.startswith('<strong>') and sub_part.endswith('</strong>'):
                        scontent = sub_part.replace('<strong>', '').replace('</strong>', '')
                        if scontent.startswith('[') and scontent.endswith(']'):
                            scontent = scontent[1:-1]
                        run_s = para.add_run(f"{prefix}{scontent}{suffix}")
                        apply_formatting(run_s, base_font, base_size, bold=True, color=base_color)
                        
                    elif is_sub_honorific:
                        if prefix:
                            run_pref = para.add_run(prefix)
                            apply_formatting(run_pref, base_font, base_size, bold=False, color=base_color)
                        
                        run_h = para.add_run(sub_part)
                        if 'Jameel Noori Nastaleeq' in base_font:
                            apply_formatting(run_h, base_font, base_size, bold=False, color=base_color)
                        else:
                            apply_formatting(run_h, base_font, base_size, bold=False, color=RGBColor(0xFF, 0x00, 0x66))
                        ...
                        run_text = para.add_run(f"{prefix}{sub_part}{suffix}")
                        apply_formatting(run_text, base_font, base_size, bold=False, color=base_color)
            else:
                run.text = f">{content}<"
                apply_formatting(run, base_font, base_size, bold=False, color=base_color)
