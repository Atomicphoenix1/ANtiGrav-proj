...
def resolve_original_path(file_path):
    """If the file is in a temp directory, try to find a matching file in the workspace or parent directories by name and content."""
    if not file_path:
        return None
        ...
        for root, dirs, files in os.walk(parent_workspace):
            if filename in files:
                ...
                if SCRIPT_DIR not in resolved:
                    try:
                        if os.path.getsize(resolved) == temp_size:
                            with open(resolved, "rb") as f:
                                candidate_prefix = f.read(4096)
                            if candidate_prefix == temp_prefix:
                                return resolved
                    except Exception:
                        continue
    return file_path
...
client = genai.Client(api_key=GEMINI_API_KEY, http_options=types.HttpOptions(timeout=180000))
...
9."ممنوع تكرار وسم المتحدث <speaker>قَالَ الشَّارِحُ هَدَاهُ اللَّهُ:</speaker> عند بداية كل فقرة جديدة إذا كان المتحدث مستمرًا في الكلام ولم يتغير. يُوضع الوسم فقط عند تغيّر المتحدث (مثلاً من الشارح إلى المصنف أو العكس) أو عند بداية المجلس."
...
def execute_pipeline(op_mode, scope, single_audio, multi_audio, folder_input, 
    ...
    # ---------------------------------------------------------
    # -1. MERGE TEXT FILES MODE
    # ---------------------------------------------------------
    if op_mode == "Merge Text Files":
        if multi_txt:
            logs = update_log(logs, f"📝 Merging {len(multi_txt)} text files in order of selection...")
            yield logs
            
            merged_content = ""
            for idx, f in enumerate(multi_txt):
                try:
                    with open(f.name, "r", encoding="utf-8") as file_obj:
                        content = file_obj.read()
                    if idx > 0 and not merged_content.endswith("\n"):
                        merged_content += "\n"
                    merged_content += content
                    logs = update_log(logs, f"   ➕ Merged: {os.path.basename(f.name)}")
                    yield logs
                except Exception as e:
            ...
            save_dir = SCRIPT_DIR
            if multi_txt:
                first_file_resolved = resolve_original_path(multi_txt[0].name)
                first_file_dir = os.path.dirname(first_file_resolved)
                if first_file_dir and "temp" not in first_file_dir.lower() and "appdata" not in first_file_dir.lower():
                    save_dir = first_file_dir
                    
            export_path = os.path.join(save_dir, "gemmerge.txt")
            logs = write_file_with_log(export_path, merged_content, logs)
            logs = update_log(logs, f"🎉 Merging completed successfully! Exported to: {export_path}")
            yield logs
        else:
            logs = update_log(logs, "❌ Error: Merge Text Files mode requires text files to be uploaded/selected.")
            yield logs
        return
    ...
    def process_lesson_folder(lesson_folder, ft_model_ref, seg_time):
        ...
        for part_dir in part_dirs:
            ...
            if part_raw_transcripts:
                ...
                # Pre-export all raw chunks first so they are saved to disk immediately
...
def toggle_inputs(op, scope, split):
    ...
    return [
        ...
        gr.update(visible=is_gemini_bulk or is_merge),  # multi_txt
        ...
        gr.update(visible=not is_merge)   # scope_mode
    ]
...
with gr.Blocks(title="Super Uploader V8") as demo:
    ...
    with gr.Row():
        with gr.Column():
            op_mode = gr.Radio(["Whisper Only", "MP3 Splitter", "Gemini Only", "Whisper + Gemini", "SRT Alignment", "Merge Text Files"], value="Whisper + Gemini", label="1. Select Operation")
