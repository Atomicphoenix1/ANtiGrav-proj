...
N8N_MP3_WEBHOOK_URL = "https://atomicphoenix1.app.n8n.cloud/webhook-test/6973bd50-c567-4e5b-8c56-51d59318dadd"
import requests # نحتاج لمكتبة ريكويستس هنا أيضاً
...
def convert_to_mp3(input_file):
    ...
    try:
        ...
        # حفظ مسار الملف الصوتي الأخير لاستخدامه في الويب هوك
        with open("latest_mp3.txt", "w", encoding="utf-8") as f:
            f.write(os.path.abspath(output_path))
        
        # إرسال الملف إلى n8n فوراً في الخلفية باستخدام Thread لعدم تعطيل الواجهة
        if N8N_MP3_WEBHOOK_URL:
            import threading
            def upload_task():
                try:
                    print(f"[+] Sending MP3 to n8n in background...")
                    with open(output_path, 'rb') as f_mp3:
                        files = {'mp3': (os.path.basename(output_path), f_mp3, 'audio/mpeg')}
                        # نستخدم timeout كبير للسماح برفع الملفات الكبيرة
                        r = requests.post(N8N_MP3_WEBHOOK_URL, files=files, timeout=600)
                        if r.status_code in [200, 201]:
                            print("[!] MP3 upload finished successfully.")
                        else:
                            print(f"[-] MP3 upload failed with status: {r.status_code}")
                except Exception as ex:
                    print(f"[-] MP3 background upload error: {ex}")
